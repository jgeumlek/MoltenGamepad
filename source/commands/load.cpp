...
int do_load(moltengamepad* mg, std::vector<token> &command) {
  if (command.size() < 4) {std::cout<<LOAD_USAGE<<std::endl;return -1;}
  if (command.at(1).value != "profiles" || command.at(2).value != "from") {std::cout<<LOAD_USAGE<<std::endl;return -1;}
  ...
  if (file.fail()) {std::cout<<"could not open file"<<std::endl; return -2;}
  shell_loop(mg,file);
  file.close();
  return 0;
}
